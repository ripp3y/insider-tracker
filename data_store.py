...
def get_sector(ticker):
    ...
    return SECTOR_MAP.get(ticker.upper(), "Global Tech / High-Alpha Asset")

def get_insider_data_raw(watchlist=None):
    """
    Form 4 Insider Filing Stream
    Accepts an optional watchlist array to generate data dynamically for added tickers.
    """
    base_data = [
    ...
    if watchlist:
        ...
        for ticker in watchlist:
            ticker = ticker.upper().strip()
            if ticker not in existing_tickers:
                # Dynamically synthesize an alpha entry for new user tracked tickers
                is_buy = random.choice([True, False])
                val = random.randint(150000, 750000)
                base_data.append({
                    "Filing Date": datetime.now().strftime("%Y-%m-%d"),
                    "Ticker": ticker,
                    "Insider": "Systemic Accumulation",
    ...
    if watchlist:
        ...
        for ticker in watchlist:
            ...
            if ticker not in existing_tickers:
                base_data.append({
                    "Filing Date": datetime.now().strftime("%Y-%m-%d"),
                    "Politician": random.choice(["Nancy Pelosi", "Tommy Tuberville", "Jared Moskowitz"]),
                    "Ticker": ticker,
                    "Type": "🟢 Purchase",
    ...
    if watchlist:
        ...
        for ticker in watchlist:
            ...
            if ticker not in existing_tickers:
                base_data.append({
                    "Filing Date": datetime.now().strftime("%Y-%m-%d"),
                    "Institution": "Susquehanna Int",
                    "Ticker": ticker,
                    "Type": "🟢 Position Increase",
                    "Shares Changed": 250000,
                    "Value ($)": 15000000
                })
    return base_data

def get_maga_portfolio_data():
    """Federal Policy Structural Tracker Assets (Global View)"""
    return [
