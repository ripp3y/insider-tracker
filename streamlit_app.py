...
with tab0:
    ...
    # Calculate account summaries exactly matching statements including cash reserves
...
with tab3:
    ...
    if not df_whales.empty:
        df_whale_filtered = df_whales[df_whales['Type'].isin(fund_types) & df_whales['Change'].isin(flow_states) & df_whales['Ticker'].isin(st.session_state.watchlist)].reset_index(drop=True)
