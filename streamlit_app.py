...
def fetch_whale_block_trades(ticker):
    """
    Analyzes 15-minute bar telemetry over the last 5 sessions to isolate
    multi-million dollar institutional blocks and classify direction.
    """
    try:
        tk = yf.Ticker(ticker)
        ...
        # Calculate dollar-volume per bar to locate anomalies
        hist['Dollar_Volume'] = hist['Volume'] * hist['Close']
        avg_bar_vol = hist['Dollar_Volume'].mean()
        
        # Block Definition: Any 15m window trading > 2.5x the average volume
...
# -----------------------------------------------------------------------------
# 2. APPLICATION INTERFACE NAVIGATION (Tab Setup)
# -----------------------------------------------------------------------------
st.title("Asymmetry: Risk & Alpha Dashboard")

# Creating independent presentation layers
...
with tab1:
    ...
    st.markdown(
        "Tracking structural blind spots where short-sellers are heavily exposed "
...
with tab2:
    ...
    st.markdown("Isolating anomalous trading spikes exceeding 2.5x normal volume to track active smart-money movement.")
    ...
    st.markdown("Cross-comparing real-time risk profiles, forward multiples, and pullbacks across your hardware ecosystem.")
    ...
    with st.spinner("Compiling full sector matrix data..."):
        for ticker in infra_watchlist:
            try:
                ...
                # Calculate pullback from market peak
